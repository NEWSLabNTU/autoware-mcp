...
class TestSystemMonitor(unittest.TestCase):
    ...
    def test_check_ros2_health(self, mock_get_manager):
        ...
        async def run_test():
            ...
            self.assertEqual(len(health["nodes"]), 2)
            self.assertEqual(len(health["topics"]), 2)
    ...
    def test_check_autoware_health(self, mock_get_config):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async def run_test():
                ...
                self.assertTrue(health["setup_bash_exists"])
                self.assertEqual(health["launch_files_found"], 1)
                self.assertEqual(len(health["errors"]), 0)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('autoware_mcp.health.psutil'):
                ...
                async def run_test():
                    ...
                    # Overall status should be degraded (no nodes, no setup.bash)
                    self.assertEqual(status["overall_status"], HealthStatus.DEGRADED)
