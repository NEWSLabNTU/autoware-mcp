    ...
    async def check_autoware_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.autoware.workspace_path:
                health["workspace_exists"] = self.config.autoware.workspace_path.exists()
            else:
            ...
            if autoware_status["autoware_running"]:
                ...
                active_components = sum(1 for nodes in autoware_status["autoware_nodes"].values() if nodes)
                ...
                if active_components >= 3:  # At least 3 component types active
                    ...
                    health["status"] = HealthStatus.DEGRADED
                    health["errors"].append("Autoware topics found but no nodes categorized")
            else:
                health["status"] = HealthStatus.UNHEALTHY
