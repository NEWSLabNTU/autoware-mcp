...
class ROS2Manager:
    ...
    async def get_topic_hz(self, topic_name: str, duration: float = 5.0) -> Dict[str, Any]:
    ...
    async def echo_topic(self, topic_name: str, count: int = 1) -> Dict[str, Any]:
        ...
        try:
            result = await self.run_command(
                ["ros2", "topic", "echo", topic_name, "--once" if count == 1 else f"--times {count}"],
                timeout=10.0
            )
            if result["returncode"] == 0:
                return {
    ...
    async def check_autoware_nodes(self) -> Dict[str, Any]:
        ...
        for node in nodes:
            ...
            if "perception" in node_lower or "lidar" in node_lower or "camera" in node_lower:
                autoware_nodes["perception"].append(node)
            elif "planning" in node_lower or "behavior" in node_lower or "motion" in node_lower:
                autoware_nodes["planning"].append(node)
            elif "control" in node_lower or "vehicle" in node_lower:
                autoware_nodes["control"].append(node)
            elif "localization" in node_lower or "pose" in node_lower or "ekf" in node_lower:
        ...
        autoware_topics = []
        topic_patterns = ["/tf", "/map", "/pointcloud", "/image", "/twist", "/trajectory", 
                         "/control", "/planning", "/perception", "/localization", "/diagnostics"]
